...
export default function ProfileClient({ username, initialTree }) {
  const [tree, setTree] = useState(initialTree);
  const [loading, setLoading] = useState(!initialTree);
  const [notFound, setNotFound] = useState(!initialTree && initialTree !== undefined);

  useEffect(() => {
    // If initialTree was passed from the server, no client fetch needed
